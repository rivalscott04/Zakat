...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('zakat_rule_parameters', function (Blueprint $table) {
            ...
            $table->unique(['zakat_rule_id', 'parameter_code']);
        });
        Schema::create('zakat_fitrah_configurations', function (Blueprint $table) {
            $table->ulid('id')->primary();
            $table->ulid('zakat_rule_id');
            $table->string('staple_type', 20);
            $table->decimal('quantity', 20, 8);
            $table->string('unit', 20);
            $table->decimal('cash_equivalent', 20, 8)->nullable();
        ...
        Schema::create('zakat_livestock_configurations', function (Blueprint $table) {
            ...
            $table->decimal('minimum_quantity', 20, 8);
            $table->decimal('maximum_quantity', 20, 8)->nullable();
            $table->decimal('zakat_quantity', 20, 8);
            $table->string('zakat_unit', 20);
            $table->text('description')->nullable();
            $table->timestamps();
            $table->foreign('zakat_rule_id')->references('id')->on('zakat_rules')->cascadeOnDelete();
        });
    ...
    {
        Schema::dropIfExists('zakat_livestock_configurations');
        Schema::dropIfExists('zakat_agriculture_configurations');
        Schema::dropIfExists('zakat_fitrah_configurations');
        Schema::dropIfExists('zakat_rule_parameters');
        Schema::table('zakat_reference_values', fn (Blueprint $table) => $table->dropColumn(['name', 'effective_at', 'expires_at']));
    }
};
