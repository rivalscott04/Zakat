...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('bank_statements', function (Blueprint $table) {
            ...
            $table->string('statement_number', 30);
            ...
            $table->ulid('imported_by')->nullable();
            $table->timestamp('imported_at')->nullable();
            $table->timestamps();
            $table->unique(['organization_id', 'statement_number']);
            $table->index(['organization_id', 'bank_account_id', 'period_start']);
        });
        Schema::create('bank_transactions', function (Blueprint $table) {
            $table->ulid('id')->primary();
            $table->ulid('organization_id');
            $table->ulid('bank_statement_id');
            $table->ulid('bank_account_id');
            ...
            $table->string('duplicate_status', 30)->default('NEW');
            $table->timestamps();
            $table->unique(['organization_id', 'bank_account_id', 'transaction_reference']);
        ...
        Schema::create('reconciliation_matches', function (Blueprint $table) {
            ...
            $table->ulid('reconciliation_transaction_id');
            $table->string('match_type', 20);
            $table->decimal('matched_amount', 18, 2);
            $table->decimal('confidence_score', 5, 2)->default(0);
            $table->ulid('matched_by')->nullable();
        ...
        Schema::create('reconciliation_sessions', function (Blueprint $table) {
            ...
            $table->ulid('organization_id');
            $table->ulid('bank_account_id');
            $table->string('session_number', 30);
            $table->date('period_start');
            $table->date('period_end');
            $table->decimal('opening_balance', 18, 2)->default(0);
            $table->decimal('closing_balance', 18, 2)->default(0);
            ...
            $table->string('status', 20)->default('DRAFT');
            $table->ulid('started_by')->nullable();
            $table->timestamp('started_at')->nullable();
            $table->timestamp('completed_at')->nullable();
            $table->timestamps();
            $table->unique(['organization_id', 'session_number']);
            $table->index(['organization_id', 'status']);
        });
        Schema::create('reconciliation_adjustments', function (Blueprint $table) {
            $table->ulid('id')->primary();
            $table->ulid('organization_id');
            $table->ulid('reconciliation_session_id');
            $table->ulid('bank_transaction_id')->nullable();
    ...
    {
        foreach (['reconciliation_adjustments', 'reconciliation_sessions', 'reconciliation_matches', 'reconciliation_transactions', 'bank_transactions', 'bank_statements', 'bank_accounts'] as $table) {
